...
bool NetUtils::verboseOutput = true;

void NetUtils::ReportError(std::string  whichFunc)
{
	int errorCode = 0;
#ifdef _WIN32
...
//------------------------------------------------------------------------------
int NetUtils::FillTheirInfo(addrinfo *who, SOCKET daSocket)
{
...
//------------------------------------------------------------------------------ 
int NetUtils::SendDataTCP(SOCKET daSocket, const char *msg)//for stream sockets
{
...
//------------------------------------------------------------------------------   
int NetUtils::SendDataUDP(SOCKET daSocket, const char *msg, int dataLen, addrinfo *whomToSend)//for datagram sockets
{
...
//------------------------------------------------------------------------------
void NetUtils::CloseConnection(SOCKET daSocket)
{
	MyCloseSocket(daSocket);
}
//------------------------------------------------------------------------------   
int NetUtils::ChangeToIsBlocking(SOCKET daSocket, bool isBlocking)// Change the socket mode on the listening socket from blocking to non-block 
{
...
//------------------------------------------------------------------------------
bool NetUtils::HasRecivedData(SOCKET daSocket)
{
	FD_ZERO(&read_fds);
...
float NetUtils::ReadFloatFromBuffer(unsigned char* buffer, size_t &index)
{
...
void NetUtils::ReadHeaderFromBuffer(unsigned char* buffer, unsigned char header[5])
{
	header[0] = buffer[0];
	header[1] = buffer[1];
	header[2] = buffer[2];
...
unsigned char* NetUtils::ReadStringFromBuffer(unsigned char* buffer, size_t size, size_t &index)
{
...
void NetUtils::WriteFloatToBuffer(float x, unsigned  char* buffer, size_t &index)
{
...
void NetUtils::WriteHeaderToBuffer(unsigned char* header, unsigned char* buffer)
{
	buffer[0] = header[0];
	buffer[1] = header[1];
	buffer[2] = header[2];
...
void NetUtils::WriteStringToBuffer(char* stringInput, unsigned char* buffer, size_t size, size_t &index)
{
	for(size_t i = index; i < (index+size); i++)
...
void NetUtils::MyCloseSocket(SOCKET daSocket)
{
#ifdef _WIN32
...
void NetUtils::print(string output)
{
	if(verboseOutput)
	{
